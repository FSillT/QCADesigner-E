...
LicenseFile=$ADD_INNOHOME$\share\doc\QCADesigner-2.2\COPYING
...
[Files]
Source: "$ADD_INNOHOME$\*"; DestDir: "{app}"; Flags: recursesubdirs
...
Name: "{group}\QCADesignerE"; Filename: "{app}\bin\QCADesignerE.exe"; IconFilename: "{app}\share\QCADesignerE\pixmaps\QCADesignerE.win32.ico"; MinVersion: 1, 0
...
begin
  Exists := RegQueryStringValue (HKLM, 'Software\Wow6432Node\GTK2-Runtime\', 'InstallationDirectory', GtkPath);
  if not Exists then begin
    Exists := RegQueryStringValue (HKLM, 'Software\Wow6432Node\GTK\2.0', 'Path', GtkPath);
  end;
  Result := Exists
end;
